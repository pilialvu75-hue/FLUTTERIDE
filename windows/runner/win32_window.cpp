...
///
/// Redefined here because the value doesn't exist in PC's Windows SDK yet.
...
// Attempts to enable full DPI scaling on the given window handle.
static void EnableFullDpiSupportIfAvailable(HWND hwnd) {
  HMODULE user32_module = LoadLibraryA("User32.dll");
  if (!user32_module) {
    return;
  }
  typedef BOOL (*EnableNonClientDpiScaling)(HWND hwnd);
...
static void UpdateTheme(HWND const window) {
  // NOTE: Registry key requires Windows 10 build 18362 or later.
  DWORD light_mode;
...
bool Win32Window::Show() {
  return ShowWindow(window_handle_, SW_SHOW) != FALSE;
}
...
// Retrieves the Win32Window instance associated with |window|.
// static
